...
* (10 points) Generic indexing with filters.
  ...
  In order for this task to count your implementation should handle all JSON values, have all basic filters, and all basic constructors.

* (10 points) Value constructors with filters.
  This is complementary to the task with basic value constructors -- implement the constructors for arrays e.g. `[.items[].name]`, objects (`{user}`, `{(.[]) : null}`).
  ...
  In order for this task to count your implementation should handle all JSON values, have all basic filters, and all object constructors.
...
* (10 points) [Try-catch expressions](https://jqlang.org/manual/#try-catch) `try op1 catch expr` which tries to execute `op1` and if exception appears, returns `expr`.
  In order for this task to count your implementation should handle all JSON values and have all basic filters.

* (10 points) [Name binding](https://jqlang.org/manual/#variable-symbolic-binding-operator) `expr as $id | op`, which allow you to bind the value expr to identifier `id` before passing it further to `op`. For this task you only have to implement basic variable binding, `expr as $id`.
    In order for this task to count your implementation should handle all JSON values and have all basic filters.
...
* (10 points) Pattern-matching binding. In the manual this is referred to as "destructing". This requires you to bind multiple variables within the same pattern. For example `expr as {username: $name, posts : [$fpost, $spost]} | op`, which will destruct object generated by expression and make variables `name`, `fpost`, and `spost` available in `op`.
    In order for this task to count, you should have regular name-binding from the previous task and simple value constructors implemented.
...
* (15 points) [Functions](https://jqlang.org/manual/#defining-functions), which allows you to define syntactical functions in jq filters.
  In order for this task to count your implementation should handle all JSON values, have all basic filters, and simple object constructors.
...
1. Define a datatype for `JSON` and type class instances on week 3. We will release solutions to parts of this task that aren't graded in week 4.
...
5. Write a parser for primitive values: booleans, numbers, strings.
   Once again, we will release solutions to parts of this task that aren't graded in week 5.
